...
- **描述/意图**：对 AI 规划类操作（增工作表/字段/视图/工作流），需要用户提供业务描述
...
**增 · 工作流**
```bash
python3 scripts/hap/incremental/add_workflow.py \
    --app-id <appId> \
...
**删 · 工作流**
```bash
python3 workflow/scripts/delete_workflow.py --workflow-id <workflowId>
```
...
| 模块 | 状态 | 实现文件 |
|------|------|---------|
| 查询（工作表/字段/工作流） | ✅ 可用 | MCP / 已有脚本 |
| 增 · 工作流 | ✅ 可用 | `incremental/add_workflow.py` |
...
| 改 · 工作流节点 | 🔧 Phase 3 | `incremental/modify_workflow.py` |
| 删 · 工作表/字段/工作流 | ✅ 可用 | 已有脚本 |

---

## 注意事项

- **工作流创建**：需要 `config/credentials/auth_config.py`（Web Cookie 认证），若缺失会报错
