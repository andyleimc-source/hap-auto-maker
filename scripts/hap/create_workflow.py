#!/usr/bin/env python3
"""
Create a new (unpublished) HAP workflow by replaying the core request from HAR.
...
def create_once(args: argparse.Namespace, account_id: str, authorization: str, cookie: str) -> dict:
    ...
    get_process_publish_url = "https://api.mingdao.com/workflow/process/getProcessPublish"
    ...
    process_add_resp = post_json(process_add_url, payload, headers)
    ...
    if process_add_resp.get("status") == 1:
        ...
        if company_id and process_id:
            ...
            add_workflow_headers = dict(headers)
            add_workflow_headers["Referer"] = (
                f"https://www.mingdao.com/workflowedit/{process_id}"
            )
            add_wf_resp = post_json(
            ...
            if args.worksheet_id:
                publish_resp = get_json(
                    f"{get_process_publish_url}?processId={process_id}",
                    headers,
                )
                start_node_id = ""
                ...
                if start_node_id:
                    ...
                    save_node_resp = post_json(save_node_url, save_node_payload, headers)
                    print(
...
def main() -> int:
    ...
    try:
        result = create_once(args, account_id, authorization, cookie)
