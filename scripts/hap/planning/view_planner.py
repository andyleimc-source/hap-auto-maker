...
3) 看板(1)：适合有明确流转阶段的表（项目/任务/订单/工单等）；必须有多状态单选字段(type=9 或 type=11)作为看板列；检查框(type=36)/等级(type=28)只有2值，不能用
4) 甘特图(5)：适合有时间跨度的项目/计划/合同/排期；需要开始日期+结束日期两个字段
5) 层级(2)：适合有父子关系的表（类目/组织架构/物料BOM等）；需要自关联字段(type=29)
6) 日历(4)：适合以日期为核心的表（排班/预约/日程/事件等）；需要日期字段
7) 画廊(3)：适合以图片为主的表（商品/案例/员工档案/设计稿等）；有附件或图片字段时优先考虑
...
4) 看板(1)：适合有明确流转阶段的表（项目/任务/订单/工单等）；必须设 viewControl 为多状态单选字段(type=9 或 type=11)ID；检查框(type=36)/等级(type=28)绝对不能用于看板
5) 甘特图(5)：适合有时间跨度的项目/计划/合同；需要开始日期+结束日期两个日期字段
6) 层级(2)：适合有父子关系的表（类目/组织架构等）；需要自关联字段(type=29, dataSource=本表)
7) 日历(4)：适合以日期为核心的表（排班/预约/日程等）；需要日期字段，在 postCreateUpdates 中设 calendarcids
8) 画廊(3)：适合以图片为主的表（商品/案例/设计稿等）；有附件字段时优先考虑
