...
def suggest_views(
    ...
    # ── 日历(4)：有业务日期字段即直接推荐，无需 AI 判断 ─────────────────────────
    SYSTEM_DATE_KEYWORDS = {"创建时间", "修改时间", "更新时间", "系统时间"}
    dates = classified_fields.get("date", [])
    business_dates = [f for f in dates
                      if f.get("name", "") not in SYSTEM_DATE_KEYWORDS]
    # 兜底：如果全是系统字段但确实有日期，仍用第一个
    calendar_date = business_dates[0] if business_dates else (dates[0] if dates else None)
    if calendar_date:
        suggestions.append({
            "viewType": 4, "name": "日历视图",
            "reason": f"有业务日期字段「{calendar_date['name']}」，适合日历浏览",
            "calendarcid": calendar_date["id"],
            "candidate": False,  # 直接推荐，不需 AI 二次判断
        })
...
**日历(4)**：已由系统自动确认推荐，无需 AI 判断。只要有业务日期字段（截止日期、计划日期等）即创建。
  ⚠️ 日历视图已在「确定推荐」列表中，直接保留即可，不要移除。

**甘特图(5)**：有开始+结束两个日期字段，且记录了「过程/进度」的工作表即应采纳（默认 ✅）。
  ✅ 适合：任务、项目、计划、排产、维修记录、审批流程、版本发布、活动筹备
  ❌ 仅当两个日期字段明确不代表起止跨度（如「入职日期」+「离职日期」混在员工档案）时才拒绝。

**资源视图(7)**：有成员字段 + 两个日期字段，且工作表是「任务/排班/分配/排期」类型即应采纳。
  ✅ 适合：任务分配、排班表、项目资源排期、工单分配
  ❌ 不适合：员工档案、合同（有成员但不是时间线排期）
...
1) viewType 必须是整数：0(表格/列表), 1(看板), 3(画廊), 4(日历), 5(甘特图), 7(资源), 8(地图)
   ...
   viewType=0 额外表格视图只有一种情况可以创建：有明确分组字段（单选 type=9/11），
   通过 groupsetting 展示分组。否则与"全部"视图无区别，禁止创建。
2) 积极策略：每个工作表应规划 2-5 个有实际业务价值的视图，类型多样化。
⚠️ 看板、日历、甘特、资源视图可以同时存在于同一工作表。满足候选条件的视图都应规划，不要因为已有某种视图就跳过其他候选。
...
2. **规划新视图**：额外规划 2-5 个有业务价值的视图（看板/日历/甘特图/画廊等），**必须至少 1 个非表格视图（viewType≠0）**，不同工作表应有差异化的视图组合
...
4) 看板(viewType=1)：仅字段语义是「阶段流转」（状态/阶段/审批）时适合，必须有单选字段(type=9/11)作为 viewControl。
   ❌ 优先级/紧急程度/等级/类型/分类字段一律不用看板。
5) 日历(viewType=4)：有业务日期字段即应创建（默认 ✅）。只有字段明确是纯系统字段（创建时间/修改时间）且无其他业务日期时才跳过；postCreateUpdates 中设 calendarcids
6) 甘特图(viewType=5)：有开始+结束两个日期字段且工作表记录过程/进度时应创建（默认 ✅）。任务/项目/计划/维修/审批等均适合。
7) 资源视图(viewType=7)：有成员字段 + 两个日期字段且工作表是任务/排班/分配类型时应创建。
