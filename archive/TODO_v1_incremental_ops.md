...
- [x] `scripts/hap/incremental/add_chart.py` — 复用 chart_planner + pipeline_charts

### 修改编排脚本（2 个）
- [x] `scripts/hap/incremental/modify_view.py` — 修改视图 advancedSetting
- [x] `scripts/hap/incremental/modify_workflow.py` — 增/改/删节点，复用 30 种 node registry

### 删除视图 CLI 封装（1 个）
- [x] `scripts/hap/delete_view.py` — 从 delete_default_views.py 提取 delete_view() 封装
...
| **Phase 3** | `modify_view.py` + `modify_workflow.py` + `add_chart.py` + `delete_view.py` | 4 | P2 | ✅ 完成 |
