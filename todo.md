...
### 截图发现的具体问题（2026-04-04 晚）

**WF-A 记录操作：**
- [ ] 图25: 获取单条员工 — "服务异常，请稍后重试"，节点配置可能参数错误
- [ ] 图26: 校准数据 — "未设置可执行的动作"，需要配置具体的校准字段
- [ ] 图27: 删除记录 — "选择删除对象"为空（请选择），需要设置 selectNodeId 或数据源
- [ ] 图28: 填写审批意见 — "未设置可填写字段"，formProperties 传了但没生效
- [ ] 图28: 发起审批 — "审批流程异常，请删除后重新配置"

**WF-C3 流程控制：**
- [ ] 图29: 整个工作流"程序错误"，编辑器崩溃（delay_until saveNode 500 导致）

**WF-D 开发者与AI：**
- [ ] 图30: 代码块 — 空白，没有代码内容
- [ ] 图31: JSON解析 — "获取JSON对象"数据源为空（请选择）
- [ ] 图32: API请求 — URL 为空
- [ ] 图33: AI生成文本 — "选择AI模型"为空，提示词为空
- [ ] 图34: AI生成对象 — "模型未开启或已删除"，提示词为空
- [ ] 图35: AI Agent — 提示词为空，工具为空

### 根本问题

当前对工作流节点的理解停留在"能创建"层面，缺乏对每个节点**功能含义和必填配置**的深入理解。

### 下一步（明天继续）

- [ ] **先读帮助文档**：研究项目下的工作流节点功能说明文档，理解每种节点的用途和必填参数
- [ ] **逐个抓包盘点**：和图表/视图一样，由用户手动配置每种节点并抓包，获取正确的 saveNode 参数
- [ ] **更新节点模块**：把抓包得到的正确参数写入 `workflow/nodes/` 各模块的 build 函数
- [ ] **重新创建验证工作流**：用正确参数创建可发布的工作流
