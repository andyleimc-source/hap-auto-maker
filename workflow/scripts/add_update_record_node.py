#!/usr/bin/env python3
"""
Add a "更新记录" action node to an existing HAP workflow.

Reference HAR:
- action/创建更新记录节点.har
- POST https://api.mingdao.com/workflow/flowNode/add        (actionId="2")
- POST https://api.mingdao.com/workflow/flowNode/saveNode
- GET  https://api.mingdao.com/workflow/process/publish     (optional)

Key difference from 新增记录 (add_new_record_node.py):
  - actionId is "2" (not "1")
  - selectNodeId is REQUIRED: the node whose record will be updated
  - No separate appId in saveNode (target worksheet is inferred from selectNodeId)

Typical usage (fields from HAR recording):
    python3 scripts/add_update_record_node.py \\
        --process-id     '69b5015d96f51d55edda03f0' \\
        --prev-node-id   '69b5015d96f51d55edda03f1' \\
        --select-node-id '69b5015d96f51d55edda03f1' \\
        --fields '[
            {"addType":0,"fieldId":"69aead71d777aea8806b9328","type":2,"enumDefault":0,"fieldValue":"21212","fieldValueId":"","nodeId":"","nodeAppId":""},
            {"addType":0,"fieldId":"69aead71d777aea8806b932a","type":16,"enumDefault":0,"fieldValue":"","fieldValueId":"nowTime","nodeId":"5d39140d381d42d20db0c4da","nodeName":"系统","fieldValueName":"当前时间","fieldValueType":16,"nodeTypeId":100,"appType":100,"actionId":"","isSourceApp":false,"sourceType":0},
            {"addType":0,"fieldId":"69aead71d777aea8806b932c","type":9,"enumDefault":0,"fieldValue":"61d1d1d1-9616-4655-9a27-955dfe9d794f","fieldValueId":"","nodeId":""}
        ]' \\
        --publish

selectNodeId: the node that provides the record to be updated.
  Usually the trigger node (start node) ID.

Field value formats:
  - Static text:       {"fieldId":"...","type":2,"enumDefault":0,"fieldValue":"hello"}
  - Dynamic (from another node): {"fieldId":"...","fieldValue":"$<nodeId>-<fieldId>$"}
  - System time:       {"fieldValueId":"nowTime","nodeId":"5d39140d381d42d20db0c4da","nodeName":"系统","fieldValueName":"当前时间","fieldValueType":16,"nodeTypeId":100,"appType":100}
  - SingleSelect:      {"type":9,"fieldValue":"<option-key-uuid>"}
  - Empty (clear):     {"fieldValue":""}
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Add a 更新记录 action node to an existing workflow."
    )
    parser.add_argument(
        "--process-id",
        required=True,
        help="Workflow process ID (e.g. 69b5015d96f51d55edda03f0).",
    )
    parser.add_argument(
        "--prev-node-id",
        required=True,
        help="ID of the node this new node should be inserted after.",
    )
    parser.add_argument(
        "--select-node-id",
        required=True,
        help=(
            "Node ID that provides the record to be updated. "
            "Usually the trigger node (start node) ID."
        ),
    )
    parser.add_argument(
        ...
        default="",
        help=(
            "Target worksheet ID. Required for auto-fetching fields when --fields is not provided. "
            "Example: 69aead71d777aea8806b931e"
        ),
    )
    parser.add_argument(
        "--name",
        default="更新记录",
        help="Display name for the node (default: 更新记录).",
    )
    parser.add_argument(
        "--fields",
        default="",
        help=(
            ...
            "Each item: {addType, fieldId, type, enumDefault, fieldValue, ...}. "
            "Dynamic ref format: \"$<nodeId>-<fieldId>$\". "
            "If omitted and --app-id is provided, fields are auto-fetched from the server."
        ),
    )
    parser.add_argument(
        "--filters",
        default="[]",
        help="JSON array of filter conditions (default: []).",
    )
    parser.add_argument(
        "--publish",
        action="store_true",
...
def load_json_arg(arg: str) -> list:
...
def run_once(args: argparse.Namespace, account_id: str, authorization: str, cookie: str) -> dict:
    ...
    # If --fields is provided, use it directly.
    # Otherwise auto-fetch from getAppTemplateControls (requires --app-id).
    if args.fields:
        ...
        print(
            ...
            f"count={len(controls_resp.get('data') or [])}",
            file=sys.stderr,
        )
        ...
        fields = []
        print("[debug] no --fields and no --app-id; saving node with empty fields", file=sys.stderr)
    ...
    return {
        ...
        "select_node_id": args.select_node_id,
        "name": args.name,
