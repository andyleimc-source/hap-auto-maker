...
🎯 节点搭配建议（让工作流更贴近真实业务）：
  ...
  - 每个工作流应包含 2~4 个节点，至少使用 1 个非 add/update_record 节点（如 notify、branch、approval 等）
...
3. 每个工作流必须有 2~4 个 action_nodes，且至少一个节点跨表（target 与触发表不同），至少一个非数据操作节点（notify/branch/approval/delay_duration/copy/calc）
