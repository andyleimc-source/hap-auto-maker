...
fieldValue 值的填写规则：
  ...
  • 日期/时间（type=15/16）：留空 ""（系统自动填当前时间）⚠️ 禁止使用 {{NOW}}/{{NOW_DATE}}/{{CURRENT_DATE}} 等占位符，HAP 不识别这些格式，会显示 Invalid date
...
7. time_triggers 的 fields 中禁止使用 {{{{trigger.xxx}}}} 语法；frequency 必须是整数（分钟数）：1440=每天，10080=每周，43200=每月。禁止使用字符串如 "daily"/"weekly"
